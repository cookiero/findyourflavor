...
async function requestAiAnalysis(files,context){
  ...
  const controller=new AbortController(),timeout=setTimeout(()=>controller.abort(),18000);let response;
...
function fallbackRecipe(flavor,context={}){
  ...
  return{
    ...
    base_analysis:`이번 여행의 시작에는 한 가지 장면보다 여행 전체를 감싼 마음의 온도가 먼저 있었던 것 같아요. 그래서 그 첫인상을 ${parts[0][3]}에 담았어요. 쿠키의 베이스가 나머지 재료를 품듯, 처음 품었던 마음이 뒤에 이어질 색과 기억을 편안하게 받쳐 주는 역할을 해요.`,
    ...
    cube_analysis:`여행이 끝난 뒤에도 기억은 거창한 결론보다 작은 조각으로 먼저 돌아오곤 하잖아요. 이번 여행에서 오래 남기로 한 기억을 ${parts[2][3]}에 담았어요. 크림 안에서 한입마다 모습을 드러내는 조각처럼, 문득 떠오르는 순간이 마지막 여행의 리듬까지 자연스럽게 이어 줘요.`,
    topping_analysis:`이번 여행이 움직인 속도와 멈춘 방식은 마지막 여운을 만든 것 같아요. 그래서 그 리듬을 ${parts[3][3]}으로 마무리했어요. 앞에서 쌓인 온도와 기억을 덮지 않으면서, 돌아온 뒤에도 이번 여행만의 박자가 또렷하게 남도록 해 주거든요.`,
    final_bake:`이번 여행은 처음 품었던 마음에서 시작해, 그 안에서 만난 기억들을 하나씩 남겨둔 시간 같아요. 흐르는 동안에는 몰랐던 장면도 돌아와 생각하면 다른 의미로 다가오곤 하잖아요. 처음의 온도와 마지막 여운 사이에 생긴 작은 변화들이 이번 여행만의 이야기가 된 것 같고요. 아마 이번 여행은 그런 맛으로 오래 남지 않을까요?`
  };
}
...
async function beginFinding(file,fromFriend=false){
  ...
  if(!fromFriend){try{photoPalette=await analyzePhotoColors(selected)}catch{photoPalette=null}photoExif=[];const memory=await resolveLayerAnswers(getJourneyMemory('main'));flavor=deterministicFlavor(photoPalette,hash,memory);const aiPromise=requestAiAnalysis(selected,{destination:document.getElementById('destinationInput').value.trim(),date:document.getElementById('dateInput').value,palette:photoPalette,selectedFlavor:flavor.name,memory}).catch(()=>null);await runBakeSequence(flavor);aiInsight=await aiPromise}
...
async function beginFriendFinding(){
  ...
  const aiPromise=requestAiAnalysis(selected,{destination:document.getElementById('friendDestinationInput').value.trim(),date:document.getElementById('friendDateInput').value,palette:friendPalette,exif:friendExif,selectedFlavor:flavor.name,memory:getJourneyMemory('friend')}).catch(()=>null);await runBakeSequence(flavor);friendAi=await aiPromise;
