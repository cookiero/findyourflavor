...
let layerStageUrls=[];
function canvasObjectUrl(canvas){return new Promise((resolve,reject)=>canvas.toBlob(blob=>blob?resolve(URL.createObjectURL(blob)):reject(new Error('cookie render failed')),'image/png'))}
async function buildLayerStages(memory){
  layerStageUrls.forEach(url=>URL.revokeObjectURL(url));layerStageUrls=[];
  const selected=chosenLayers(memory),size=820,canvas=document.createElement('canvas');canvas.width=size;canvas.height=size;const ctx=canvas.getContext('2d',{alpha:true}),stages=[],base=cleanLayer(await loadLayerImage(layerAsset('base',selected.base))),pawMask=cleanLayer(await loadLayerImage(`assets/layers/footprint/${footprintFiles[selected.base]||footprintFiles[layerDefaults.base]}`)),pawScale=.58,pawSize=size*pawScale,pawInset=(size-pawSize)/2;
  ctx.drawImage(base,0,0,size,size);stages.push(await canvasObjectUrl(canvas));
  for(const layer of ['cream','cube','topping']){const overlay=cleanLayer(await loadLayerImage(layerAsset(layer,selected[layer]))),masked=document.createElement('canvas');masked.width=size;masked.height=size;const m=masked.getContext('2d',{alpha:true});m.drawImage(overlay,0,0,size,size);m.globalCompositeOperation='destination-in';m.drawImage(base,0,0,size,size);m.globalCompositeOperation='destination-out';m.drawImage(pawMask,pawInset,pawInset,pawSize,pawSize);m.beginPath();m.arc(size*.715,size*.715,size*.052,0,Math.PI*2);m.fill();m.globalCompositeOperation='source-over';ctx.drawImage(masked,0,0);stages.push(await canvasObjectUrl(canvas));await new Promise(requestAnimationFrame)}
  layerStageUrls=stages;return stages;
}
...
function showScreen(id){
  ...
  document.body.classList.toggle('is-baking',id==='loading');
...
function shareUrl(){const payload={version:3,flavorId:currentFlavor.id,analysis:shareAnalysis(currentFlavor,memoryAnalysis())},url=new URL(location.href);url.search='';url.hash=`share=${encodeShare(payload)}`;return url.toString()}
