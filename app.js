...
function startBaking(){const stage=document.getElementById('bakingStage'),cookie=document.getElementById('bakingCookie'),copy=document.getElementById('loadingText'),messages=['사진에서 여행의 재료를 찾는 중...','그날의 빛을 반죽에 섞는 중...','여행의 온도로 굽는 중...'];let index=0;stage.dataset.phase='dough';cookie.src='assets/cookie-ring.png';copy.textContent=messages[0];const phaseOne=setTimeout(()=>{stage.dataset.phase='mix';copy.textContent=messages[1]},1300),phaseTwo=setTimeout(()=>{stage.dataset.phase='bake';copy.textContent=messages[2]},2800),messageLoop=setInterval(()=>{if(stage.dataset.phase==='bake'){index=(index+1)%3;copy.textContent=messages[index]}},1900);return()=>{clearTimeout(phaseOne);clearTimeout(phaseTwo);clearInterval(messageLoop)}}
...
const waitlistFormMarkup=`<span class="waitlist-kicker">COOKIE:RO BAKERY — COMING SOON</span><h3>아직 오븐은<br>예열 중이에요.</h3><p class="waitlist-copy">Cookie:Ro Bakery가 문을 열면,<br>당신의 여행으로 만든 첫 번째 쿠키를<br>가장 먼저 구워드릴게요.<br><br>지금 이름을 남겨두면<br>오븐이 준비되는 순간 알려드릴게요.</p><form class="waitlist-form"><label><span>1. 이름</span><input name="name" type="text" autocomplete="name" maxlength="40" placeholder="이름을 적어주세요" required></label><label><span>2. 연락받을 곳</span><input name="contact" type="text" autocomplete="email" maxlength="120" placeholder="이메일 주소 / 전화번호 / 카카오톡 ID 중 하나" required></label><label class="waitlist-honey" aria-hidden="true"><span>Website</span><input name="website" type="text" tabindex="-1" autocomplete="off"></label><p class="waitlist-note">남겨주신 연락처는 Bakery 오픈 알림에만 사용돼요.</p><button type="submit">🍪 첫 쿠키 예약해두기</button><p class="waitlist-status" role="status" aria-live="polite"></p></form>`;
const waitlistSuccessMarkup=`<div class="waitlist-success"><span class="cookie-mark" aria-hidden="true">🍪</span><h3>반죽해두었어요.</h3><p>Cookie:Ro Bakery가 문을 여는 날,<br>당신의 여행부터 가장 먼저 구워드릴게요.</p></div>`;
...
function initWaitlist(){let complete=false;try{complete=localStorage.getItem('cookiero-waitlist')==='complete'}catch{}document.querySelectorAll('[data-waitlist]').forEach(card=>{card.innerHTML=complete?waitlistSuccessMarkup:waitlistFormMarkup;if(complete)return;const form=card.querySelector('form'),status=card.querySelector('.waitlist-status'),button=form.querySelector('button');form.addEventListener('submit',async event=>{event.preventDefault();const data=new FormData(form),name=String(data.get('name')||'').trim(),contact=String(data.get('contact')||'').trim();if(!name||contact.length<2){status.textContent='이름과 연락받을 곳을 모두 알려주세요.';return}button.disabled=true;button.textContent='반죽에 이름을 새기는 중…';status.textContent='';try{const response=await fetch('/api/waitlist',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name,contact,website:String(data.get('website')||''),source:card.closest('.compare-screen')?'friend-result':'my-result'})}),result=await response.json().catch(()=>null);if(!response.ok)throw new Error(result?.message||'오븐 연결이 잠시 느려요. 조금 뒤 다시 시도해 주세요.');markWaitlistComplete()}catch(error){status.textContent=error.message;button.disabled=false;button.textContent='🍪 첫 쿠키 예약해두기'}})})}
