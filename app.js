...
let photos=[]; let friendPhotos=[]; let currentFlavor=flavors[2]; let friendBase=null; let photoPalette=null; let photoExif=[]; let aiInsight=null; let friendPalette=null; let friendExif=[]; let friendAi=null; let sharedData=null; let currentCookieId=null; let friendCookieId=null;let mainPhotoAnalysisPromise=null,mainPhotoAnalysisKey='';
...
function cleanLayer(image,size=760){const temp=document.createElement('canvas');temp.width=size;temp.height=size;const t=temp.getContext('2d',{willReadFrequently:true});t.drawImage(image,0,0,size,size);const pixels=t.getImageData(0,0,size,size),d=pixels.data;for(let i=0;i<d.length;i+=4){const max=Math.max(d[i],d[i+1],d[i+2]),min=Math.min(d[i],d[i+1],d[i+2]);if(d[i+3]&&max-min<10&&min>205)d[i+3]=0}t.putImageData(pixels,0,0);return temp}
async function buildLayerStages(memory){const size=760,selected=chosenLayers(memory),canvas=document.createElement('canvas');canvas.width=size;canvas.height=size;const ctx=canvas.getContext('2d'),stages=[],base=cleanLayer(await loadLayerImage(layerAsset('base',selected.base)),size),pawMask=cleanLayer(await loadLayerImage(`assets/layers/footprint/${footprintFiles[selected.base]||footprintFiles[layerDefaults.base]}`),size),pawScale=.58,pawSize=size*pawScale,pawInset=(size-pawSize)/2;ctx.drawImage(base,0,0);stages.push(canvas.toDataURL('image/png'));for(const layer of ['cream','cube','topping']){await new Promise(resolve=>setTimeout(resolve,0));const overlay=cleanLayer(await loadLayerImage(layerAsset(layer,selected[layer])),size),masked=document.createElement('canvas');masked.width=size;masked.height=size;const m=masked.getContext('2d');m.drawImage(overlay,0,0);m.globalCompositeOperation='destination-in';m.drawImage(base,0,0);m.globalCompositeOperation='destination-out';m.drawImage(pawMask,pawInset,pawInset,pawSize,pawSize);m.beginPath();m.arc(size*.715,size*.715,size*.052,0,Math.PI*2);m.fill();m.globalCompositeOperation='source-over';ctx.drawImage(masked,0,0);stages.push(canvas.toDataURL('image/png'))}return stages}
...
function previewFile(input,index,isFriend=false){
  ...
  else{photos[index]=file;scheduleMainPhotoAnalysis();syncFindButton()}
}
function previewBatch(input,isFriend=false){const files=[...(input.files||[])].filter(file=>file.type.startsWith('image/')).slice(0,3);if(!files.length)return;const selector=isFriend?'input[data-friend-slot]':'input[data-slot]',inputs=[...document.querySelectorAll(selector)],target=isFriend?friendPhotos:photos;target.length=0;files.forEach((file,index)=>{target[index]=file;const label=inputs[index].closest('.photo-slot'),img=label.querySelector('img');img.src=URL.createObjectURL(file);label.classList.add('has-image')});if(isFriend)syncFriendButton();else{scheduleMainPhotoAnalysis();syncFindButton()}}
...
async function fingerprint(file){
  if(!file)return Date.now();const chunk=32768,head=new Uint8Array(await file.slice(0,chunk).arrayBuffer()),tail=file.size>chunk?new Uint8Array(await file.slice(Math.max(chunk,file.size-chunk),file.size).arrayBuffer()):new Uint8Array(),data=[...head,...tail];let hash=2166136261;
  for(let i=0;i<data.length;i+=Math.max(1,Math.floor(data.length/1200)))hash=Math.imul(hash^data[i],16777619);
  hash=Math.imul(hash^file.size,16777619);return Math.abs(hash);
}
...
async function sampleImage(file){return new Promise((resolve,reject)=>{const img=new Image(),url=URL.createObjectURL(file);img.onload=()=>{const scale=Math.min(1,112/Math.max(img.width,img.height)),w=Math.max(1,Math.round(img.width*scale)),h=Math.max(1,Math.round(img.height*scale));const c=document.createElement('canvas');c.width=w;c.height=h;const ctx=c.getContext('2d',{willReadFrequently:true,alpha:false});ctx.drawImage(img,0,0,w,h);const d=ctx.getImageData(0,0,w,h).data,out=[];for(let i=0;i<d.length;i+=32){if(d[i+3]>180)out.push([d[i],d[i+1],d[i+2]])}URL.revokeObjectURL(url);resolve(out)};img.onerror=()=>{URL.revokeObjectURL(url);reject(new Error('image read failed'))};img.src=url})}
async function analyzePhotoColors(files){let pixels=(await Promise.all(files.map(sampleImage))).flat();if(!pixels.length)return null;if(pixels.length>3500)pixels=pixels.filter((_,i)=>i%Math.ceil(pixels.length/3500)===0);const picks=[.08,.34,.62,.9].map(q=>pixels[Math.min(pixels.length-1,Math.floor(pixels.length*q))].slice());let groups=[];for(let step=0;step<5;step++){groups=picks.map(()=>({sum:[0,0,0],n:0}));for(const p of pixels){let bi=0,bd=Infinity;picks.forEach((c,i)=>{const d=(p[0]-c[0])**2+(p[1]-c[1])**2+(p[2]-c[2])**2;if(d<bd){bd=d;bi=i}});const g=groups[bi];g.sum[0]+=p[0];g.sum[1]+=p[1];g.sum[2]+=p[2];g.n++}groups.forEach((g,i)=>{if(g.n)picks[i]=g.sum.map(v=>Math.round(v/g.n))});await new Promise(requestAnimationFrame)}const total=groups.reduce((s,g)=>s+g.n,0);let result=groups.map((g,i)=>({rgb:picks[i],raw:g.n/total*100})).filter(x=>x.raw>.15).sort((a,b)=>b.raw-a.raw);const floors=result.map(x=>Math.floor(x.raw));let remaining=100-floors.reduce((a,b)=>a+b,0);for(let i=0;i<remaining;i++)floors[i%floors.length]++;return result.map((x,i)=>{const hex='#'+x.rgb.map(v=>v.toString(16).padStart(2,'0')).join('');return[colorName(x.rgb),hex,floors[i]]})}
function photoSetKey(files){return files.filter(Boolean).map(file=>`${file.name}:${file.size}:${file.lastModified}`).join('|')}
function scheduleMainPhotoAnalysis(){const selected=photos.filter(Boolean),key=photoSetKey(selected);mainPhotoAnalysisKey=key;photoPalette=null;mainPhotoAnalysisPromise=new Promise(resolve=>{const run=()=>analyzePhotoColors(selected).then(result=>resolve({key,result})).catch(()=>resolve({key,result:null}));if('requestIdleCallback'in window)requestIdleCallback(run,{timeout:350});else setTimeout(run,30)})}
async function preparedMainPhotoPalette(selected){const key=photoSetKey(selected);if(mainPhotoAnalysisPromise&&mainPhotoAnalysisKey===key){const prepared=await mainPhotoAnalysisPromise;if(prepared.key===key)return prepared.result}return analyzePhotoColors(selected)}
...
function layerThemeFlavor(memory,hash=0){const groups=[layerCatalog.base,layerCatalog.cream,layerCatalog.cube,layerCatalog.topping],selected=chosenLayers(memory),scores=[0,0,0,0,0];['base','cream','cube','topping'].forEach((layer,index)=>{const hit=groups[index].findIndex(item=>normalizedName(item)===normalizedName(selected[layer]));if(hit>=0)scores[hit]++});const best=Math.max(...scores),ties=scores.map((score,index)=>score===best?index:-1).filter(index=>index>=0);return flavors[ties[hash%ties.length]]}
const localAnswerMeaning={
  'pink cloud base':['포근한 온도','다정한 시작','편안한 마음'],'fresh morning base':['맑은 온도','산뜻한 시작','가벼운 마음'],'sun_kissed base':['생기 찬 온도','뜨거운 시작','힘찬 마음'],'mellow base':['느긋한 온도','차분한 시작','고요한 마음'],'deep night base':['깊은 온도','몽환적 시작','진한 마음'],
  'blue cloud cream':['부드러운 빛','파스텔 색','편안한 여백'],'white chocolate cream':['투명한 빛','밝은 색','맑은 공기'],'soda cream':['선명한 색','또렷한 대비','톡 쏘는 공기'],'matcha cream':['자연의 색','고요한 빛','잔잔한 공기'],'chocolate ganache':['그윽한 색','낮은 빛','잔잔한 깊이'],
  'marshmellow cubes':['함께한 기억','다정한 장면','나눈 시간'],'cheese cubes':['새로운 발견','처음 본 풍경','남은 호기심'],'mango cubes':['감각의 기억','생생한 결','맛있는 순간'],'white chocolate cubes':['오래 머문 장면','조용한 기억','깊은 시선'],'brownie cubes':['진하게 남은 일','깊은 기억','밤의 여운'],
  'rainbow sprinkles':['흩어진 리듬','다정한 움직임','가벼운 여운'],'lemon zest':['또렷한 리듬','가벼운 걸음','산뜻한 여운'],'popping candy':['즉흥의 리듬','자유로운 전환','통통 튄 여운'],'roasted choco pearls':['느린 리듬','천천한 걸음','긴 여운'],'crunchy chocolate balls':['깊어진 리듬','이어진 시간','진한 여운'],
  '포근하고 다정한 온도':['포근한 온도','다정한 시작','편안한 마음'],'맑고 산뜻한 온도':['맑은 온도','산뜻한 시작','가벼운 마음'],'생기 있고 뜨거운 온도':['생기 찬 온도','뜨거운 시작','힘찬 마음'],'차분하고 깊은 온도':['차분한 온도','깊은 시작','고요한 마음'],
  '부드러운 파스텔빛':['부드러운 빛','은은한 색','편안한 여백'],'투명하고 밝은 빛':['투명한 빛','밝은 색','맑은 공기'],'선명하고 강한 색':['선명한 색','또렷한 대비','강한 인상'],'그윽하고 낮은 채도':['그윽한 색','낮은 채도','잔잔한 깊이'],
...
function memoryAnalysis(memory=currentLayerMemory){
  ...
  return{base_keywords:memory.layerKeywords?.base,cream_keywords:memory.layerKeywords?.cream,cube_keywords:memory.layerKeywords?.cube,topping_keywords:memory.layerKeywords?.topping,base_analysis:memory.layerReasons.base,cream_analysis:memory.layerReasons.cream,cube_analysis:memory.layerReasons.cube,topping_analysis:memory.layerReasons.topping,final_bake:memory.journeyNarrative||memory.journeyTagline};
}
...
function renderResult(flavor){
  ...
  renderRecipe('recipeSections','finalBake',flavor,memoryAnalysis());
  ...
  const narrative=currentLayerMemory?.journeyNarrative||currentLayerMemory?.journeyTagline||'서로 다른 네 가지 기억이 부드럽게 이어져, 오래 꺼내 보고 싶은 한 입이 되었어요.';const summaryLine=document.querySelector('#resultRecipe .taste-summary-line');if(summaryLine){summaryLine.textContent=narrative;summaryLine.classList.add('journey-narrative')}const summaryName=document.querySelector('#resultRecipe .taste-summary strong');if(summaryName)summaryName.hidden=true;const recipeFooter=document.querySelector('#resultRecipe footer');if(recipeFooter)recipeFooter.hidden=true;
...
async function beginFinding(file,fromFriend=false){
  ...
  if(!fromFriend){try{photoPalette=await preparedMainPhotoPalette(selected)}catch{photoPalette=null}currentLayerMemory=await resolveLayerAnswers(getJourneyMemory('main'),photoVisualSignals(photoPalette));currentLayerStages=await buildLayerStages(currentLayerMemory);flavor=layerThemeFlavor(currentLayerMemory,hash);aiInsight=null;await runBakeSequence()}
...
async function beginFriendFinding(){
  ...
  const hash=await fingerprint(selected[0]);friendAi=null;currentLayerMemory=await resolveLayerAnswers(getJourneyMemory('friend'),photoVisualSignals(friendPalette));const flavor=layerThemeFlavor(currentLayerMemory,hash);currentLayerStages=await buildLayerStages(currentLayerMemory);await runBakeSequence();
...
function shareUrl(){const payload={version:4,flavorId:currentFlavor.id,cookieName:cookieNameFor(currentLayerMemory),layers:chosenLayers(currentLayerMemory),analysis:shareAnalysis(currentFlavor,memoryAnalysis())},url=new URL(location.href);url.search='';url.hash=`share=${encodeShare(payload)}`;return url.toString()}
document.getElementById('shareButton').addEventListener('click',async()=>{
  ...
  try{const url=shareUrl(),data={title:'Cookie:Ro — Find the Flavor',text:`내 여행으로 구운 ${cookieNameFor(currentLayerMemory)}! 네 가지 레이어를 같이 봐줘.`,url};if(navigator.share)await navigator.share(data);else{await navigator.clipboard.writeText(url);document.getElementById('shareStatus').textContent='사진 없이 쿠키 설명만 담긴 링크를 복사했어요!'}}catch(e){if(e.name!=='AbortError')toast(e.message||'공유 링크를 만들지 못했어요.')}finally{button.classList.remove('is-loading');button.innerHTML='같이 여행한 친구에게 보내기 <span>↗</span>'}
});
...
function getCookieRecord(card){
  ...
  const memory=currentLayerMemory||getJourneyMemory(friendFlow?'friend':'main'),analysis=friendFlow?friendAi:memoryAnalysis(memory),recipe={...fallbackRecipe(flavor,{palette:friendFlow?friendPalette:photoPalette,destination}),...(analysis||{})};return{cookieId:friendFlow?friendCookieId:currentCookieId,destination,travelDate,flavorId:'layer-cookie',flavorName:cookieNameFor(memory),tagline:memory.journeyTagline,base:memory.resolvedChoices?.base||memory.base,cream:memory.resolvedChoices?.cream||memory.cream,cube:memory.resolvedChoices?.cube||memory.cube,topping:memory.resolvedChoices?.topping||memory.topping,baseKeywords:recipe.base_keywords,creamKeywords:recipe.cream_keywords,cubeKeywords:recipe.cube_keywords,toppingKeywords:recipe.topping_keywords,baseAnalysis:recipe.base_analysis,creamAnalysis:recipe.cream_analysis,cubeAnalysis:recipe.cube_analysis,toppingAnalysis:recipe.topping_analysis,finalBake:recipe.final_bake,baseReason:memory.layerReasons?.base,creamReason:memory.layerReasons?.cream,cubeReason:memory.layerReasons?.cube,toppingReason:memory.layerReasons?.topping,baseNote:memory.baseNote,creamNote:memory.creamNote,cubeNote:memory.cubeNote,toppingNote:memory.toppingNote,answerVersion:memory.answerVersion,photosStored:false,storageVersion:7};
}
