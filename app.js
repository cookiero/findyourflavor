...
function renderRecipe(sectionId,finalId,flavor,analysis){
  ...
  const stickers=['밀대.png','짤주머니.png','믹스.png','요리사모자.png'];recipes[flavor.id].forEach((part,index)=>{const row=document.createElement('section');row.className='recipe-part';const icon=document.createElement('span');icon.className='recipe-icon';icon.textContent=part[2];const body=document.createElement('div');body.className='recipe-copy';const small=document.createElement('small');small.textContent=`${part[0]} — ${part[1]}`;const title=document.createElement('h4');title.textContent=part[3];const p=document.createElement('p');p.textContent=copy[fields[index]];const sticker=document.createElement('img');sticker.className='recipe-sticker';sticker.src=`assets/bake/${stickers[index]}`;sticker.alt='';body.append(small,title,p);row.append(icon,body,sticker);holder.append(row)});final.textContent=copy.final_bake;
}
