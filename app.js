...
async function requestAiAnalysis(files,context){
  ...
  const {destination,date,palette,selectedFlavor,memory}=context,place=findPlace(destination);
  ...
  const controller=new AbortController(),timeout=setTimeout(()=>controller.abort(),40000);let response;
  try{response=await fetch('/api/analyze',{method:'POST',headers:{'Content-Type':'application/json'},signal:controller.signal,body:JSON.stringify({destination,date,palette,curated,selectedFlavor,memory})})}finally{clearTimeout(timeout)}
...
async function runBakeSequence(flavor){const stage=document.getElementById('bakingStage'),cookie=document.getElementById('bakingCookie'),copy=document.getElementById('loadingText'),name=bakeNames[flavor.id],steps=[['base','사진에서 발견한 색을\n크림으로 섞는 중…'],['cream','오래 남을 기억을\n한 조각씩 올리는 중…'],['cube','여행의 리듬을\n마지막으로 뿌리는 중…'],['topping','YOUR COOKIE IS READY\nFOR ITS PAW PRINT.']];await pause(700);for(const [phase,message] of steps){stage.dataset.phase=phase;cookie.src=`assets/bake/${name}_${phase}.png`;copy.textContent=message;await pause(850)}}
async function finishBaking(flavor,next){const stage=document.getElementById('bakingStage'),cookie=document.getElementById('bakingCookie'),copy=document.getElementById('loadingText'),title=document.getElementById('loadingTitle'),stamp=document.getElementById('pawStamp'),crumbs=document.getElementById('bakingCrumbs');crumbs.src=`assets/bake/${bakeNames[flavor.id]} crumbs.png`;stage.dataset.phase='stamp';stamp.setAttribute('aria-hidden','false');copy.textContent='Cookie:Ro의 도장을 찍었어요!';playBakingChime('oven');await pause(900);cookie.src=flavor.file;stage.dataset.phase='final';title.textContent='YOUR TRIP HAS BEEN BAKED.';copy.textContent='당신의 여행이 하나의 쿠키로 완성됐어요.';playBakingChime('jar');await pause(1050);stamp.setAttribute('aria-hidden','true');next()}
...
const fallbackMeanings={
  base:{'포근하고 다정한 온도':[['포근한 출발','다정한 마음','편안한 온도'],'서두르기보다 마음을 편하게 두고 여행을 받아들이는 쪽에 가까웠던 것 같아요.'],'맑고 산뜻한 온도':[['산뜻한 출발','가벼운 마음','맑은 온도'],'복잡한 마음을 잠시 내려놓고 가볍게 시작하고 싶었던 여행처럼 느껴져요.'],'생기 있고 뜨거운 온도':[['힘찬 출발','생기 찬 마음','뜨거운 온도'],'새로운 시간을 망설이기보다 온몸으로 만나고 싶었던 여행에 가까워 보여요.'],'차분하고 깊은 온도':[['고요한 출발','깊은 마음','차분한 온도'],'많이 채우기보다 천천히 머물며 마음의 결을 들여다보는 시간이 중요했던 것 같아요.']},
  cream:{'부드러운 파스텔빛':[['부드러운 색','포근한 공기','은은한 대비'],'강한 한 장면보다 여러 순간이 부드럽게 이어지는 색감을 골라준 것 같아요.'],'투명하고 밝은 빛':[['투명한 빛','맑은 공기','가벼운 대비'],'탁하지 않고 환하게 열린 인상이 이번 여행을 떠올리는 데 중요했던 것 같아요.'],'선명하고 강한 색':[['선명한 색','또렷한 공기','강한 대비'],'한눈에 들어오는 생생함과 또렷한 인상이 오래 남은 여행처럼 느껴져요.'],'그윽하고 낮은 채도':[['그윽한 색','잔잔한 공기','낮은 대비'],'화려함보다 차분하게 스며드는 분위기가 더 오래 머무는 여행이었던 것 같아요.']},
  cube:{'사람과 함께한 순간':[['함께한 시간','나눈 기억','다정한 조각'],'무엇을 보았는지만큼 누구와 시간을 나누었는지가 이 여행을 오래 남게 한 것 같아요.'],'처음 발견한 풍경':[['새로운 발견','처음의 감각','호기심 한 조각'],'익숙한 답을 확인하기보다 처음 만난 것에 마음이 움직였던 여행처럼 느껴져요.'],'맛과 촉감 같은 감각':[['감각의 기억','생생한 조각','몸에 남은 순간'],'설명보다 몸으로 느낀 감각이 먼저 기억을 불러오는 여행이었던 것 같아요.'],'혼자 오래 머문 장면':[['오래 머문 시간','조용한 기억','깊은 한 조각'],'빠르게 지나가기보다 혼자 충분히 머문 시간이 여행의 중심에 있었던 것 같아요.']},
  topping:{'천천히 머문 리듬':[['느린 걸음','여유로운 흐름','긴 여운'],'많이 움직이는 것보다 한곳에 충분히 머무는 속도가 잘 맞았던 여행 같아요.'],'계획대로 흐른 리듬':[['고른 걸음','단단한 흐름','편안한 여운'],'미리 그려둔 흐름을 따라가며 여행을 안정적으로 쌓아가는 기분이 중요했던 것 같아요.'],'즉흥적으로 튄 리듬':[['자유로운 걸음','뜻밖의 전환','톡 튄 여운'],'계획에서 조금 벗어나는 순간까지 여행의 재미로 받아들인 듯해요.'],'밤까지 이어진 진한 리듬':[['이어진 걸음','깊어진 흐름','진한 여운'],'하루를 일찍 닫기보다 시간이 깊어질수록 여행의 결을 더 오래 따라간 것 같아요.']}
};
function fallbackChoice(layer,memory){const selected=memory?.resolvedChoices?.[layer]||memory?.[layer],entry=fallbackMeanings[layer][selected];return entry||[[`${layer==='base'?'마음의 온도':layer==='cream'?'여행의 색':layer==='cube'?'남은 기억':'여행의 리듬'}`,'나만의 선택','오래 남은 결'],'이번 여행에서 고른 답에는 그 시간을 기억하는 나름의 기준이 담겨 있는 것 같아요.']}
function fallbackRecipe(flavor,context={}){
  const parts=recipes[flavor.id],memory=context.memory||{},destination=context.destination||'그곳',base=fallbackChoice('base',memory),cream=fallbackChoice('cream',memory),cube=fallbackChoice('cube',memory),topping=fallbackChoice('topping',memory),palette=(context.palette||flavor.colors).slice(0,3),paletteText=palette.map(color=>`${color[0]} ${color[2]}%`).join(', '),note=line=>memory?.[`${line}Note`]?' 직접 남겨준 이야기를 함께 놓고 보면, 이 선택에는 겉으로 보이는 분위기보다 조금 더 개인적인 이유가 있었던 듯하고요.':'';
  return{base_keywords:base[0],cream_keywords:cream[0],cube_keywords:cube[0],topping_keywords:topping[0],base_analysis:`${destination}으로 향할 때는 ${base[1]}${note('base')} 그래서 여행의 첫 온도로 고른 재료는 ${parts[0][3]}예요. 이 베이스가 처음의 마음을 편안하게 받쳐 주면서, 다음 크림이 다른 표정을 더할 자리를 만들어줘요.`,cream_analysis:`사진 전체에서 실제로 읽힌 중심 색은 ${paletteText}였어요. ${cream[1]}${note('cream')} 그래서 여행의 색으로 고른 재료는 ${parts[1][3]}이에요. 앞의 베이스가 가진 첫 온도를 감싸면서도, 뒤의 큐브가 기억의 조각으로 또렷하게 드러나게 해주는 크림이거든요.`,cube_analysis:`이번 여행에서 오래 남는 방식은 ${cube[1]}${note('cube')} 그래서 기억의 조각으로 고른 재료는 ${parts[2][3]}예요. 크고 거창한 결론보다 시간이 지난 뒤 문득 돌아오는 기억을 담기에 잘 어울리고, 마지막 토핑의 리듬으로도 자연스럽게 이어져요.`,topping_analysis:`여행이 흘러간 방식은 ${topping[1]}${note('topping')} 그래서 마지막 리듬으로 고른 재료는 ${parts[3][3]}예요. 앞에서 쌓인 온도와 색, 기억을 덮지 않으면서도 돌아온 뒤까지 이 여행만의 박자를 남겨주거든요.`,final_bake:`이번 여행은 ${base[0][0]}에서 시작해 자기만의 속도로 천천히 모양을 갖춰간 것 같아요. 그 안에서 ${cube[0][0]}이 오래 남을 중심이 되었고, ${topping[0][0]}이 여행의 마지막 표정을 만들어줬고요. 떠날 때 품었던 마음과 돌아온 뒤 기억하는 방식이 꼭 같지는 않다는 점도 여행답게 느껴져요. 그래서 처음에는 한 가지 인상으로 보였던 시간이, 돌아볼수록 조금씩 다른 이야기를 꺼내주는 것 같아요. 아마 ${destination}에서의 시간도 그런 맛으로 오래 남지 않을까요?`};
}
function renderRecipe(sectionId,finalId,flavor,analysis){
  ...
  const friendResult=sectionId==='friendRecipeSections',sharedResult=sectionId==='sharedRecipeSections',context={palette:friendResult?friendPalette:photoPalette,destination:document.getElementById(friendResult?'friendDestinationInput':'destinationInput')?.value.trim(),memory:sharedResult?null:friendResult?friendResolvedMemory:currentResolvedMemory};
...
function shareAnalysis(flavor,analysis){const context={palette:photoPalette,destination:document.getElementById('destinationInput')?.value.trim(),memory:currentResolvedMemory},copy={...fallbackRecipe(flavor,context),...(analysis||{})};return{base_keywords:copy.base_keywords,cream_keywords:copy.cream_keywords,cube_keywords:copy.cube_keywords,topping_keywords:copy.topping_keywords,base_analysis:copy.base_analysis,cream_analysis:copy.cream_analysis,cube_analysis:copy.cube_analysis,topping_analysis:copy.topping_analysis,final_bake:copy.final_bake}}
...
async function beginFinding(file,fromFriend=false){
  ...
  if(!fromFriend){try{photoPalette=await analyzePhotoColors(selected)}catch{photoPalette=null}photoExif=[];const memory=await resolveLayerAnswers(getJourneyMemory('main'));currentResolvedMemory=memory;flavor=deterministicFlavor(photoPalette,hash,memory);const aiPromise=requestAiAnalysis(selected,{destination:document.getElementById('destinationInput').value.trim(),date:document.getElementById('dateInput').value,palette:photoPalette,selectedFlavor:flavor.name,memory}).catch(()=>null);await runBakeSequence(flavor);aiInsight=await aiPromise}
...
async function beginFriendFinding(){
  ...
  try{friendPalette=await analyzePhotoColors(selected)}catch{friendPalette=null}friendExif=[];
  const hash=await fingerprint(selected[0]),flavor=deterministicFlavor(friendPalette,hash);
  friendResolvedMemory=await resolveLayerAnswers(getJourneyMemory('friend'));const aiPromise=requestAiAnalysis(selected,{destination:document.getElementById('friendDestinationInput').value.trim(),date:document.getElementById('friendDateInput').value,palette:friendPalette,selectedFlavor:flavor.name,memory:friendResolvedMemory}).catch(()=>null);await runBakeSequence(flavor);friendAi=await aiPromise;
...
const waitlistFormMarkup=`<span class="conversion-step">02 · SAVE YOUR COOKIE</span><span class="waitlist-kicker">YOUR FIRST COOKIE IS WAITING.</span><h3>Cookie:Ro Bakery가 문을 열면,<br>가장 먼저 당신의 여행을 구워드릴게요.</h3><p class="waitlist-copy">지금 발견한 <strong data-jar-flavor>Flavor</strong>와 여행의 네 가지 재료를 Cookie Jar에 담아둘게요. 정식 Cookie:Ro가 시작되면 흩어진 여행 사진을 나만의 Crumbook으로 만들어 다시 만나보세요.</p><form class="waitlist-form"><label><span>1. 이름</span><input name="name" type="text" autocomplete="name" maxlength="40" placeholder="이름을 적어주세요" required></label><label><span>2. 연락받을 곳</span><input name="contact" type="text" autocomplete="email" maxlength="120" placeholder="이메일 주소 / 전화번호 / 카카오톡 ID 중 하나" required></label><label class="waitlist-honey" aria-hidden="true"><span>Website</span><input name="website" type="text" tabindex="-1" autocomplete="off"></label><p class="waitlist-note">지금 발견한 Flavor와 분석 결과를 Cookie Jar에 보존하고, 정식 서비스 오픈 소식과 초기 이용 안내를 보내드려요. 사진은 저장하지 않아요.</p><button type="submit">🍪 내 쿠키 담아두고 먼저 초대받기</button><p class="waitlist-status" role="status" aria-live="polite"></p></form>`;
