...
- visualSignals는 사진 원본이 아니라 브라우저에서 계산한 색상 통계다. 관찰 가능한 대표색·비율·밝기·채도·대비만 사실로 사용할 수 있으며, 사진 속 사물·사람·장소·행동은 추측하지 않는다.
...
matches: BASE/CUBE/TOPPING은 자유서술이 있으면 selected와 note를 함께 보고 options 중 의미와 정서적 결이 가장 가까운 재료를 고르며, 없으면 selected를 유지한다. CREAM은 cream의 답과 visualSignals를 함께 근거로 골라야 한다. 사진 신호와 답이 다르면 어느 하나를 무시하지 말고 둘의 대비를 가장 잘 잇는 재료를 고른다.

reasons: 각 값은 자연스러운 한국어 4~6문장이다. BASE는 여행의 정서적 온도 해석 → 왜 BASE인지 → 다음 CREAM과의 관계. CREAM은 visualSignals의 대표색(필요하면 상위 2~3개 비율), 밝기·채도·대비 중 실제로 두드러진 신호와 사용자의 cream 답을 함께 해석 → 왜 CREAM인지 → 앞 BASE와 뒤 CUBE를 잇는 관계. 수치를 모두 기계적으로 나열하지 말고 의미 있는 신호만 자연스럽게 말한다. CUBE는 기억이 남은 방식 해석 → 왜 CUBE인지 → CREAM 안에서 드러나 TOPPING으로 이어지는 관계. TOPPING은 여행의 속도와 움직임 해석 → 왜 TOPPING인지 → 앞 세 레이어에 남기는 마지막 리듬과 여운. 재료의 일반 설명보다 사용자의 여행이 중심이어야 한다.
...
export default async function handler(req,res){
  ...
  const memory=req.body?.memory||{},rawSignals=req.body?.visualSignals;
  ...
  const visualSignals=rawSignals&&typeof rawSignals==='object'?{colors:Array.isArray(rawSignals.colors)?rawSignals.colors.slice(0,4).map(color=>({name:String(color?.name||'').slice(0,40),hex:/^#[0-9a-f]{6}$/i.test(color?.hex)?color.hex:'',percent:Math.max(0,Math.min(100,Number(color?.percent)||0))})):[],brightness:Math.max(0,Math.min(100,Number(rawSignals.brightness)||0)),saturation:Math.max(0,Math.min(100,Number(rawSignals.saturation)||0)),contrast:Math.max(0,Math.min(100,Number(rawSignals.contrast)||0)),brightnessLabel:String(rawSignals.brightnessLabel||'').slice(0,40),saturationLabel:String(rawSignals.saturationLabel||'').slice(0,40),contrastLabel:String(rawSignals.contrastLabel||'').slice(0,40)}:null;
  try{
    const response=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${process.env.OPENAI_API_KEY}`},body:JSON.stringify({model:'gpt-5.6-luna',store:false,instructions,input:JSON.stringify({answers:safe,visualSignals}),text:{format:{type:'json_schema',name:'cookiero_journey_interpretation',strict:true,schema}}})});
