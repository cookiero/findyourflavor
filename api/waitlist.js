...
export default async function handler(req,res){
  ...
  const allowedSources=new Set(['find_your_flavor','friend_share']);
  ...
  if(!emailPattern.test(email))return res.status(400).json({error:'이메일 주소를 확인해 주세요.'});
  ...
  try{
    const response=await fetch(`${supabaseUrl}/rest/v1/waitlist?on_conflict=email`,{
      ...
      body:JSON.stringify({name,email,source,flavor,page_url:pageUrl,updated_at:new Date().toISOString()})
    });
