...
- 사용자에게 보여주는 글에는 AI, 분석, EXIF, GPS, 좌표, 메타데이터 같은 기술 용어나 정보 출처를 쓰지 마세요.
  ...
  - base_analysis는 base/baseNote만 사용해 여행의 정서적 온도를 해석하고 왜 이 BASE인지 설명한 뒤 다음 CREAM이 더하는 역할로 이어가세요. 색 이름, 색 비율, 밝기, 사진을 언급하지 마세요.
  - cream_analysis는 브라우저가 계산한 대표색·비율과 사용자의 답을 함께 해석하고 왜 이 CREAM인지 설명한 뒤 앞 BASE와 뒤 CUBE 사이를 어떻게 잇는지 이야기하세요.
  - cube_analysis는 cube/cubeNote만 사용해 기억이 남은 방식을 해석하고 왜 이 CUBE인지 설명한 뒤 CREAM 안에서 어떤 기억의 조각으로 드러나 TOPPING으로 이어지는지 이야기하세요. 색 이름, 색 비율, 밝기, 사진을 언급하지 마세요.
  - topping_analysis는 topping/toppingNote만 사용해 여행의 속도와 움직임을 해석하고 왜 이 TOPPING인지 설명한 뒤 앞 세 레이어에 남기는 마지막 리듬과 여운으로 마무리하세요. 색 이름, 색 비율, 밝기, 사진을 언급하지 마세요.
  - 모든 문장은 색상 설명에서 끝나지 않고 “Cookie:Ro가 여행을 쿠키 레시피로 번역했다”는 인상을 주어야 합니다. 다정하지만 상투적이지 않게 쓰세요.
  - final_bake는 네 재료나 앞의 분석을 다시 요약하는 곳이 아닙니다. 여행의 시작 → 그 안에서 생긴 변화나 대비 → 기억으로 남은 방식 → 돌아본 뒤의 여운을 하나의 이야기로 엮어 5~7문장으로 쓰세요. CREAM에서 얻은 색을 전체 여행의 근거처럼 확장하지 말고, 색 이름이나 비율도 반복하지 마세요. 재료 이름은 꼭 필요할 때만 비유로 사용하고, 마지막은 “아마 이번 여행은 그런 맛으로 오래 남지 않을까요?”처럼 여백이 있는 비단정적인 문장으로 끝내세요.
