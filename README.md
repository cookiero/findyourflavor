...
- v4.4 전체본에 v4.5 아이콘·셰프 모자 패치를 적용했습니다.
- Find Your Flavor 결과를 재료 설명이 아니라 사용자 여행의 해석으로 생성합니다.
- 각 레이어는 `응답 해석 → 재료 선택 이유 → 인접 레이어와의 관계` 순서로 이야기합니다.
- 키워드는 사용자 답변별로 생성하고, 마지막에는 네 재료의 반복 요약 대신 여행 전체의 흐름을 하나의 narrative로 보여줍니다.
- AI를 사용할 수 없을 때도 객관식 선택만으로 안전한 로컬 결과를 제공하며, 입력받지 않은 사건은 만들지 않습니다.
- 공유 링크와 Cookie Jar에는 같은 개인화 결과가 저장됩니다.
