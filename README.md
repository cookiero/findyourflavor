...
- 사진을 선택한 직후 브라우저 유휴 시간에 색상 분석을 미리 수행합니다.
- 분석용 축소 크기, 픽셀 표본과 색상 군집 반복 횟수를 낮추고 각 반복 사이에 화면 렌더링 시간을 양보합니다.
- 사진 지문은 고해상도 원본 전체 대신 앞뒤의 작은 조각과 파일 크기로 계산합니다.
- Find 버튼 이후에는 미리 계산된 사진 결과를 재사용합니다.

## v4.8 변경사항

- 굽기 화면의 쿠키 합성 크기를 화면 표시 품질에 충분한 820px로 최적화했습니다.
- 합성 PNG를 동기 Data URL 대신 비동기 Blob URL로 생성해 화면 멈춤을 줄였습니다.
- 레이어 사이에 브라우저 렌더링 프레임을 양보하고, 굽는 동안 고비용 배경 노이즈·블러·그림자를 잠시 중지합니다.

## v4.7 변경사항

- 사진 원본은 브라우저 밖으로 보내지 않고 대표색·비율·밝기·채도·대비만 로컬에서 계산합니다.
- 계산된 사진 색감 신호를 CREAM 재료 선택, 개인화 키워드와 해석에 실제로 반영합니다.
- 사진에 없는 사람·사물·장소·행동·사건은 추측하지 않습니다.
